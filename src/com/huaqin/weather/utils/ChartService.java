...
import android.graphics.Paint.Align;
 ...
 {	 
     ...
     private XYSeriesRenderer mRenderer2;// 单条曲线渲染器
     ...
     */
     ...
     public void setXYMultipleSeriesDataset(String curveTitle,String curveTitle2) 
     {
         multipleSeriesDataset = new XYMultipleSeriesDataset();
         mSeries = new XYSeries(curveTitle);
         mSeries2 = new XYSeries(curveTitle2);
         multipleSeriesDataset.addSeries(mSeries);
         ...
         int labelColor, int curveColor, int gridColor) 
    {
         ...
         multipleSeriesRenderer.setRange(new double[] { 0, 8, 0, 40 });//xy轴的范围
         ...
         multipleSeriesRenderer.setBackgroundColor(Color.BLUE);//背景色
         ...
         mRenderer2.setColor(Color.BLUE);
    ...
    * 根据新加的数据，更新曲线，只能运行在主线程
     ...
     public void updateChart(List<Double> xList, List<Double> yList,List<Double> xxList, List<Double> yyList) 
     {
         for (int i = 0; i < xList.size(); i++) {
            mSeries.add(xList.get(i), yList.get(i));
            mSeries2.add(xxList.get(i), yyList.get(i));
         }
         mGraphicalView.repaint();//此处也可以调用invalidate()
     }
 }